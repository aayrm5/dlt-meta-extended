...
class PIIUpdateUtilitySilver:
    ...
    def update_pii_details_silver(self):
        attributes = [
        ...
        pii_update_spec_df = self.__get_silver_pii_update_spec_dataframe(pii_update_file_df)
    ...
    def __get_silver_pii_update_spec_dataframe(self, pii_df):
        """
        ...
        for pii_row in pii_rows:
            ...
            source_pii_fields_json = {}

            if (pii_row_columns.count("targetPiiFields") == 1) :
                source_pii_fields_json = pii_row["targetPiiFields"]
            if source_pii_fields_json:
                json_pii_fields = source_pii_fields_json.asDict()
                for piiField in json_pii_fields :
                    if(json_pii_fields[piiField]):
                        source_pii_fields[piiField] = json_pii_fields[piiField]


            if(pii_row_columns.count("silver_pii_fields") == 1) :
                target_pii_fields_json = pii_row["silver_pii_fields"]
            ...
            if target_pii_fields:
                ...
                print("No PII Transformation found for Silver. Exiting")
